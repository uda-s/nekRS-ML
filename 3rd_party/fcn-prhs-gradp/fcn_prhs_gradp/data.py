...
def discover_pairs(
    ...
    for path in data_dir.glob("fld_*_time_*_rank_*_size_*.bin"):
    ...
    return [
        FieldPairIndex(time=key[0], rank=key[1], size=key[2], x_path=inputs[key], y_path=outputs[key])
        for key in sorted(inputs)
    ]


def _read_pair(pair: FieldPairIndex, input_dim: int, output_dim: int) -> tuple[np.ndarray, np.ndarray]:
...
def load_element_dataset(
    ...
    seed: int = 12,
) -> tuple[TensorDataset, DatasetStats, list[FieldPairIndex]]:
