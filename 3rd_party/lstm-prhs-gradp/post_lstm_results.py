...
def parse_run(run_dir: Path) -> RunData | None:
    ...
    n_lstm_layers = int(model["n_lstm_layers"])
    ...
    tag = f"H{hidden}_L{n_lstm_layers}_LR{lr12:g}_{lr23:g}_B{batch}_P{ph1}_{ph2}_{ph3}"
...
def discover_runs(root: Path, recursive: bool) -> list[RunData]:
    ...
    for history_path in sorted(root.glob(pattern)):
        ...
        if run is not None:
            runs.append(run)
    unique: dict[Path, RunData] = {}
    for run in runs:
        unique[run.run_dir.resolve()] = run
...
def save_overlay(runs: list[RunData], outpath: Path, metric: str) -> None:
    ...
    for run in sorted(runs, key=lambda r: (r.hidden, r.n_lstm_layers, r.lr_phase12, r.lr_phase23)):
...
def save_heatmaps(runs: list[RunData], outdir: Path) -> None:
    ...
    layers = sorted({run.n_lstm_layers for run in runs})
    ...
    for lr12, lr23 in lr_pairs:
        ...
        index = {(run.hidden, run.n_lstm_layers): run for run in selected}
    ...
    for n_layer in layers:
        selected = [run for run in runs if run.n_lstm_layers == n_layer]
...
def main() -> None:
    ...
    parser.add_argument("--root", type=Path, default=Path("lstm_outputs_grid"))
    ...
    if not runs:
        raise RuntimeError(f"No LSTM runs found under {root}")
